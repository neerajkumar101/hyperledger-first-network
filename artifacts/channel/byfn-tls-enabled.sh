...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -f ../../docker-compose/e2e/docker-compose-e2e.yaml
  fi
}
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/leftorg1.created.com/tlsca/
  PRIV_KEY=$(ls *_sk)
  cd $CURRENT_DIR
  ...
  cd crypto-config/peerOrganizations/rightorg2.created.com/tlsca/
  PRIV_KEY=$(ls *_sk)
  cd $CURRENT_DIR
...
# and two Peer Orgs (``LeftOrg1`` & ``RightOrg2``) each managing and maintaining two peer nodes.
...
# (``peer0.leftorg1.created.com`` & ``peer0.rightorg2.created.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for LeftOrg1MSP   ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/LeftOrg1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg LeftOrg1MSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for LeftOrg1MSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for RightOrg2MSP   ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/RightOrg2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg RightOrg2MSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for RightOrg2MSP..."
    exit 1
  fi
  echo
}
...
COMPOSE_FILE=../../docker-compose/cli-config/docker-compose-cli.yaml
